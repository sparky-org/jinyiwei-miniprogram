  ...
  <div class="my_approval">
    ...
    <!-- <div class="weui-form-preview">
      ...
      <div class="weui-form-preview__ft">
        ...
        <div class="weui-form-preview__btn weui-form-preview__btn_primary" hover-class="weui-form-preview__btn_active">批准</div>
      </div>
    </div> -->
...
export default {
  ...
  methods: {
    handleAction(item, state){
      ...
      wx.showModal({
        ...
        success: async (res) => {
          ...
          if (res.confirm) {
            console.log('确定')
            const data = await post(`/agency/info/approve?workflowId=${item.workflowId}&result=${state}`);
            if(data.success){
              wx.showToast({
    ...
    // ...mapMutations(["update"]),
    async getData() {
      const data = await post(`/agency/info/getApprovingWorkflow?agencyId=${this.$store.state.userInfo.agencyId}`);
      if(data.success){
        if(data.result){
